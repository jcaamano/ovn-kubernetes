...
func TestConcurrentPopAndPush(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// check that the consumers are tracking the oldest items
			// irrespective of push order, as many old items as consumers should
			// be tracked by them so they should not be present on the backing
			// slice
