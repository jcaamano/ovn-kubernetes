...
func (c *MACBindingController) reconcileMacBindings(key string) error {
	...
	return c.reconcileMacBindingsForIPFromSource(ip, port)
}

// reconcileMacBindingsForIPFromSource mirrors the designated's (ip, mac) onto every target port.
func (c *MACBindingController) reconcileMacBindingsForIPFromSource(ip, source string) error {
	start := time.Now()
	...
	err := c.setMacBindingsForIPFromSourceToTargets(ip, source, followers)
	if err != nil {
...
func (c *MACBindingController) updateFollowers(
	...
	if logAddedBySource != nil {
		klog.V(5).Infof("Follower allocation changed: added ports: %v, removed ports: %v",
			logAddedBySource,
