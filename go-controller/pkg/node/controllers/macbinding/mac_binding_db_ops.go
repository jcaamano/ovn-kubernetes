...
const (
	...
	// mirrorSourceStaleThresholdMs is how long a mirror source's MAC_Binding may
	// go without a timestamp refresh before we suspect a backlog. The source's
	// own statctrl keeps the row fresh under normal load, so a source that is
	// stale (or missing) by the time we mirror it hints that we are falling
	// behind and a target may not have the binding when it needs it.
	mirrorSourceStaleThresholdMs = 1000
	// backlogWarningIntervalMs throttles the backlog warning so a sustained
	// backlog does not flood the log.
	backlogWarningIntervalMs = int64(time.Minute / time.Millisecond)
)

// setMacBindingsForIPFromSourceToTargets reads the source's (ip, mac) binding
// and mirrors it onto every target port.
func (c *MACBindingController) setMacBindingsForIPFromSourceToTargets(ip, source string, targets []string) error {
	...
	if age := int(time.Now().UnixMilli()) - mb.Timestamp; age > mirrorSourceStaleThresholdMs {
		c.throttleBacklogWarning("MAC_Binding for %s on source port %s is being refreshed with %dms delay", source, ip, age)
	}
...
func (c *MACBindingController) throttleBacklogWarning(format string, args ...any) {
	...
	klog.Warningf("Potential delay mirroring MAC bindings (muted for the next %dms): "+format,
		append([]any{backlogWarningIntervalMs}, args...)...,
	)
}
...
func (c *MACBindingController) allowBacklogWarning(nowMs int64) bool {
	...
	if nowMs-last < backlogWarningIntervalMs {
		return false
	}
