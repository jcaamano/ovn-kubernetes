...
func (c *MACBindingController) setMacBindingsForIPFromSourceToTargets(ip, source string, targets []string, warnDelayThresholdMs int) error {
	...
	err = c.setMacBindingsFromSourceToTargets(map[string]string{mb.IP: mb.MAC}, source, targets)
	if err != nil {
		return err
	}
...
// mirrored writes don't churn a row the target may already be refreshing on its
// own. The caller passes a fresh timestamp (time.Now). portToDatapath maps each
// target logical port to its datapath UUID.
	...
	for port, datapath := range portToDatapath {
		for ip, mac := range macBindings {
			...
			case err == nil:
				...
				// otherwise-unchanged row, which the target may already be
				// keeping fresh on its own. A changed MAC must be written
				// immediately, or the target keeps forwarding to a stale MAC
...
// warnDelayThrottleIntervalMs regardless of how many sources or IPs are affected.
