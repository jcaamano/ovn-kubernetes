...
func (m *mockSyncOps) expectCalls(g gomega.Gomega, expected []syncOpsCall, timeout ...time.Duration) {
	...
	eventually := g.Eventually(func(g gomega.Gomega) {
		calls := m.getCalls()
	...
	if len(timeout) > 0 {
		eventually = eventually.WithTimeout(timeout[0])
	}
...
func (m *mockSyncOps) AddMACBinding(ip, mac string, _ int, ports []portInfo) error {
	return m.record("AddMACBinding", ip, mac, ports)
}

func (m *mockSyncOps) UpdateMACBinding(ip, mac string, _ int, ports []portInfo) error {
	return m.record("UpdateMACBinding", ip, mac, ports)
}

func (m *mockSyncOps) DeleteAndAddMACBinding(ip, mac string, _ int, ports []portInfo) error {
...
func newTestController(t *testing.T, cfg testControllerConfig) *testController {
	...
	t.Cleanup(func() {
		close(tc.stopCh)
		controllerutil.Stop(tc.controller.macBindingReconciler)
	})
	...
	tc.controller.macBindingReconciler = controllerutil.NewReconciler(
	...
	// Simulate ListExisting delivering startup neighbors before the
	// reconciler starts so that keys are deduplicated in the queue.
	...
	// Allow events to be processed and enqueued.
	time.Sleep(100 * time.Millisecond)

	err := controllerutil.Start(
		tc.controller.macBindingReconciler,
	)
	g.Expect(err).ToNot(gomega.HaveOccurred())
	...
	tests := []struct {
		...
		},
		{
			...
			steps: []neighStep{
				...
				},
				{
					...
					expected: []syncOpsCall{
						{method: "DeleteAndAddMACBinding", ip: "10.0.0.1", mac: "aa:bb:cc:dd:ee:02", ports: []portInfo{port}},
					},
				},
			},
		},
		{
			...
			steps: []neighStep{{
				...
				expected: []syncOpsCall{
					{method: "DeleteAndAddMACBinding", ip: "fd00::1", mac: "aa:bb:cc:dd:ee:01", ports: []portInfo{port}},
				},
			}},
		},
		{
			...
			steps: []neighStep{{
				event: neighEvent{net.ParseIP("10.0.0.1"), net.HardwareAddr{0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x01}, 42, netlink.NUD_REACHABLE, unix.RTM_NEWNEIGH},
				expected: []syncOpsCall{
					{method: "DeleteAndAddMACBinding", ip: "10.0.0.1", mac: "aa:bb:cc:dd:ee:01", ports: []portInfo{
						{LogicalPort: "rtoe-GR_blue_node1", DatapathUUID: "dp-1"},
						{LogicalPort: "rtoe-GR_red_node1", DatapathUUID: "dp-2"},
					}},
				},
...
func TestHandleLinkEvent(t *testing.T) {
	...
	// Simulate a bridge recreate: new link index.
	newBridge := &netlink.Bridge{}
	newBridge.Index = 99
	newBridge.Name = "breth0"

	// Close the old channel to stop the old forwarding goroutine, then
	// create a new one for the resubscription.
	close(tc.neighCh)
	tc.neighCh = make(chan netlink.NeighUpdate, 100)

	// Update mock so the ticker's resubscribe picks up the new index.
	...
	// Simulate a REACHABLE neighbor arriving via the new subscription.
	tc.sendNeighUpdate(net.ParseIP("10.0.0.5"), net.HardwareAddr{0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0x05}, 99, netlink.NUD_REACHABLE, unix.RTM_NEWNEIGH)

	// The ticker may take up to 1s to detect the index change and
	// resubscribe, so extend the timeout.
...
func TestScan(t *testing.T) {
	...
	tests := []struct {
		...
		expectCalls   []syncOpsCall
	}{
		{
			name: "stale entry evicted",
			entries: []cacheEntrySpec{
				{ip: "10.0.0.1", mac: "aa:bb:cc:dd:ee:01", staleTime: time.Now().Add(-time.Minute)},
			},
			expectEvicted: []string{"10.0.0.1"},
		},
		{
			name: "fresh entry kept",
			entries: []cacheEntrySpec{
				{ip: "10.0.0.1", mac: "aa:bb:cc:dd:ee:01", staleTime: time.Now().Add(time.Hour)},
			},
		},
		{
			...
			entries: []cacheEntrySpec{
				{ip: "10.0.0.1", mac: "aa:bb:cc:dd:ee:01", staleTime: time.Now().Add(-time.Minute)},
				{ip: "10.0.0.2", mac: "aa:bb:cc:dd:ee:02", staleTime: time.Now().Add(time.Hour)},
			},
			expectEvicted: []string{"10.0.0.1"},
			expectCalls: []syncOpsCall{
				{method: "SyncARPFlows", ip: "10.0.0.2", mac: "aa:bb:cc:dd:ee:02"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, e := range tt.entries {
				mac := e.mac
				entry := newMacBinding(e.ip, &mac)
				entry.setStaleTime(e.staleTime)
				tc.controller.getCache().Store(e.ip, entry)
			}

			tc.controller.scan()

			for _, ip := range tt.expectEvicted {
				_, found := tc.controller.getCache().Load(ip)
				g.Expect(found).To(gomega.BeFalse(), "expected %s to be evicted", ip)
			}

			if len(tt.expectCalls) > 0 {
				tc.syncOps.expectCalls(g, tt.expectCalls)
			} else {
				tc.syncOps.expectNoCalls(g)
			}
		})
