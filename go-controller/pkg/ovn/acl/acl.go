...
func PurgeRejectRules(nbClient libovsdbclient.Client) error {
	// TODO
	// - Removing blindly reject ACLs might be problematic inf in the future we add this types of ACLs for other purposes
	...
	if err != nil {
		klog.Errorf("Error trying to remove ACLs %+v frpm port group %s: %v", acls, types.ClusterPortGroupName, err)
	}
