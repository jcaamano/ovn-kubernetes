...
const (
	defaultDenyPolicyTypeACLExtKey = "default-deny-policy-type"
	l4MatchACLExtKey               = "l4Match"
	ipBlockCIDRACLExtKey           = "ipblock_cidr"
	namespaceACLExtKey             = "namespace"
	policyACLExtKey                = "policy"
	policyTypeACLExtKey            = "policy_type"
	policyTypeNumACLExtKey         = "%s_num"
)
...
func buildACL(namespace, portGroup, name, direction string, priority int, match, action, aclLogging string, policyType knet.PolicyType) *nbdb.ACL {
	...
	if policyType != "" {
		externalIds = map[string]string{
			defaultDenyPolicyTypeACLExtKey: string(policyType),
		}
	}
