...
func (n kNetworkPolicy) getDefaultDenyData(networkPolicy *knet.NetworkPolicy, ports []string) []libovsdb.TestData {
	...
	return []libovsdb.TestData{
		&nbdb.ACL{
			UUID:      "named_acl_" + networkPolicy.Namespace + "_" + networkPolicy.Name + "_" + string(knet.PolicyTypeEgress),
			...
			Log:       false,
			Meter:     []string{types.OvnACLLoggingMeter},
			Severity:  []string{nbdb.ACLSeverityInfo},
			ExternalIDs: map[string]string{
				"default-deny-policy-type": string(knet.PolicyTypeEgress),
			},
		},
		&nbdb.ACL{
			UUID:      "named_acl_" + networkPolicy.Namespace + "_ARPallowPolicy" + "_" + string(knet.PolicyTypeEgress),
		...
		&nbdb.ACL{
			UUID:      "named_acl_" + networkPolicy.Namespace + "_" + networkPolicy.Name + "_" + string(knet.PolicyTypeIngress),
			Name:      []string{networkPolicy.Namespace + "_" + networkPolicy.Name},
			Priority:  types.DefaultDenyPriority,
			Direction: nbdb.ACLDirectionToLport,
			...
			Severity:  []string{nbdb.ACLSeverityInfo},
			ExternalIDs: map[string]string{
				"default-deny-policy-type": string(knet.PolicyTypeIngress),
			},
		},
		&nbdb.ACL{
			UUID:      "named_acl_" + networkPolicy.Namespace + "_ARPallowPolicy" + "_" + string(knet.PolicyTypeIngress),
			Name:      []string{networkPolicy.Namespace + "_ARPallowPolicy"},
			Priority:  types.DefaultAllowPriority,
			Direction: nbdb.ACLDirectionToLport,
		...
		&nbdb.PortGroup{
			...
			ACLs: []string{
				"named_acl_" + networkPolicy.Namespace + "_" + networkPolicy.Name + "_" + string(knet.PolicyTypeIngress),
				"named_acl_" + networkPolicy.Namespace + "_ARPallowPolicy" + "_" + string(knet.PolicyTypeIngress),
			},
			ExternalIDs: map[string]string{
				"name": pgHash + "_" + ingressDenyPG,
			},
		...
		&nbdb.PortGroup{
			...
			ACLs: []string{
				"named_acl_" + networkPolicy.Namespace + "_" + networkPolicy.Name + "_" + string(knet.PolicyTypeEgress),
				"named_acl_" + networkPolicy.Namespace + "_ARPallowPolicy" + "_" + string(knet.PolicyTypeEgress),
			},
			ExternalIDs: map[string]string{
				"name": pgHash + "_" + egressDenyPG,
			},
...
func (n kNetworkPolicy) getPolicyData(networkPolicy *knet.NetworkPolicy, policyPorts []string, peerNamespaces []string, tcpPeerPorts []int32) []libovsdb.TestData {
	...
	for i := range networkPolicy.Spec.Ingress {
		...
		if peerNamespaces != nil {
			...
			aclNamedUUID := "named_acl_" + aclName + "_" + policyType
		...
		for _, v := range tcpPeerPorts {
			aclNamedUUID := fmt.Sprintf("named_acl_%s_%s_tcp_%d", aclName, policyType, v)
			aclNamedUUIDs = append(aclNamedUUIDs, aclNamedUUID)
			data = append(data, &nbdb.ACL{
				UUID:      aclNamedUUID,
				Name:      []string{aclName},
