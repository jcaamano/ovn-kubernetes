...
func (n kNetworkPolicy) getDefaultDenyData(networkPolicy *knet.NetworkPolicy, ports []string) []libovsdb.TestData {
	...
	egressDenyACL := libovsdbops.BuildACL(
		...
		false,
		map[string]string{
			defaultDenyPolicyTypeACLExtKey: string(knet.PolicyTypeEgress),
		},
	)
	...
	ingressDenyACL := libovsdbops.BuildACL(
		...
		false,
		map[string]string{
			defaultDenyPolicyTypeACLExtKey: string(knet.PolicyTypeIngress),
		},
	)
...
func (n kNetworkPolicy) getPolicyData(networkPolicy *knet.NetworkPolicy, policyPorts []string, peerNamespaces []string, tcpPeerPorts []int32) []libovsdb.TestData {
	...
	for i := range networkPolicy.Spec.Ingress {
		...
		if peerNamespaces != nil {
			...
			acls = append(acls, libovsdbops.BuildACL(
				aclName,
				nbdb.ACLDirectionToLport,
				types.DefaultAllowPriority,
				...
				false,
				map[string]string{
					l4MatchACLExtKey:     "None",
					ipBlockCIDRACLExtKey: "false",
					namespaceACLExtKey:   networkPolicy.Namespace,
					policyACLExtKey:      networkPolicy.Name,
					policyTypeACLExtKey:  policyType,
		...
		for _, v := range tcpPeerPorts {
			acls = append(acls, libovsdbops.BuildACL(
				...
				false,
				map[string]string{
					l4MatchACLExtKey:     fmt.Sprintf("tcp && tcp.dst==%d", v),
					ipBlockCIDRACLExtKey: "false",
					namespaceACLExtKey:   networkPolicy.Namespace,
					policyACLExtKey:      networkPolicy.Name,
					policyTypeACLExtKey:  policyType,
	...
	ginkgo.Context("during execution", func() {
		...
		ginkgo.It("correctly creates and deletes a networkpolicy allowing a port to a local pod", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Eventually(fakeOvn.nbClient).Should(libovsdb.HaveData(expectedDataWithPolicy2...))
				// TODO
				// libovsdb test server is not doing garbage collection
				// so the actual ACLs will not be removed in the test.
				// For now, test that the policy port group is removed
				policy2PG := expectedDataWithPolicy2[len(expectedDataWithPolicy2)-1]
				gomega.Eventually(fakeOvn.nbClient).Should(gomega.Not(libovsdb.HaveData(policy2PG)))
...
func buildExpectedACL(gp *gressPolicy, pgName string, as []string) *nbdb.ACL {
	...
	externalIds := map[string]string{
		l4MatchACLExtKey:     "None",
		ipBlockCIDRACLExtKey: "false",
		namespaceACLExtKey:   gp.policyNamespace,
		policyACLExtKey:      gp.policyName,
		policyTypeACLExtKey:  gpDirection,
