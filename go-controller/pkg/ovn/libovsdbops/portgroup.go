...
func findPortGroup(nbClient libovsdbclient.Client, pg *nbdb.PortGroup) (string, error) {
	pgs := []nbdb.PortGroup{}
	err := nbClient.Where(pg).List(&pgs)
	if err != nil {
		return "", fmt.Errorf("can't find port group %+v: %v", *pg, err)
	}

	if len(pgs) > 1 {
		return "", fmt.Errorf("unexpectedly found multiple port groups: %+v", pgs)
	}

	if len(pgs) == 1 {
		return pgs[0].UUID, nil
	}
