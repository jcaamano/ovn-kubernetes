...
// findACL looks up the client cache for equivalent ACLs
...
func BuildACL(name, direction string, priority int, match, action, meter, severity string, log bool, externalIds map[string]string) *nbdb.ACL {
	uuid := buildNamedUUID()
	var nameSet []string
	if name != "" {
		nameSet = []string{fmt.Sprintf("%.63s", name)}
	}
	return &nbdb.ACL{
		UUID:        uuid,
		Name:        nameSet,
