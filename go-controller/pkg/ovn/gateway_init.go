...
func (oc *Controller) gatewayInit(nodeName string, clusterIPSubnet []*net.IPNet, hostSubnets []*net.IPNet,
	...
	for _, entry := range clusterIPSubnet {
		...
		p := func(item *nbdb.LogicalRouterStaticRoute) bool {
			return item.IPPrefix == lrsr.IPPrefix && item.Nexthop == lrsr.Nexthop && util.SliceHasStringItem(tmpRouters[0].StaticRoutes, item.UUID)
		}
	...
	for _, nextHop := range l3GatewayConfig.NextHops {
		...
		p := func(item *nbdb.LogicalRouterStaticRoute) bool {
			return item.OutputPort != nil && *item.OutputPort == *lrsr.OutputPort && item.Nexthop == lrsr.Nexthop
		}
		err := libovsdbops.CreateOrUpdateLogicalRouterStaticRoutesWithPredicate(oc.nbClient, gatewayRouter, &lrsr, p)
		if err != nil {
