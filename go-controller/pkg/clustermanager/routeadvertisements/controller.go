...
func (c *Controller) generateFRRConfiguration(
	...
	for i, router := range source.Spec.BGP.Routers {
		...
		for _, neighbor := range source.Spec.BGP.Routers[i].Neighbors {
			// If MultiProtocol is disabled (not by default) then a BGP session
			...
			// address that was used to stablish the session. Let's disallow the
			// use of DisableMP for now.
			if neighbor.DisableMP {
				return nil, fmt.Errorf("%w: DisableMP==true not supported, seen on FRRConfiguration %s/%s neighbor %s",
					errConfig,
					source.Namespace,
					source.Name,
					neighbor.Address,
