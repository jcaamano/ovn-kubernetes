...
func addPeriodAndJitter(base time.Time, period, jitter int) time.Time {
	t := time.Duration(period) * time.Second
	j := time.Duration(rand.Intn(jitter)) * time.Millisecond
	return base.Add(t).Add(j)
}
...
// probeTask represents a probing task for a node including the execution time,
// the context it is executed under and the client used for the probe
...
func (nt probeTask) Time() time.Time {
	return nt.time
}
...
type controller struct {
	...
	controller   controllerutil.Controller
	...
	// node at most
	probeTaskQueue *timequeue.TimeQueue[*probeTask]
...
func newController(port, timeout int) *controller {
	return &controller{
		...
		probeTaskQueue: timequeue.New[*probeTask](),
	}
}
