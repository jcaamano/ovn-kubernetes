...
func startOvnKube(ctx *cli.Context, cancel context.CancelFunc) error {
	...
	// Set up leader election process. User lease resource lock as configmap and
	...
	// Use a cancellable context so that we know when runOvnKube has properly
	// exited after we stopped leading.
	var stopOvnKube context.Context
	...
	lec := leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			OnStartedLeading: func(ctx context.Context) {
				...
				var ovnKubeDone context.CancelFunc
				stopOvnKube, ovnKubeDone = context.WithCancel(context.Background())
				defer ovnKubeDone()
			...
			OnStoppedLeading: func() {
				...
				cancel()
				// OnStoppedLeading might be called even if OnStartedLeading
				// wasn't, contrary to what the documentation implies.
				if stopOvnKube != nil {
					<-stopOvnKube.Done()
				}
			},
